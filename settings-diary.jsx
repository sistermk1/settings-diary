...
import { ChevronLeft, ChevronRight, X, Save, Trash2, Star, ChevronDown, Plus, Share2, Copy, Check, Upload, Film, AlertCircle, Download, Sun, Moon, Search } from 'lucide-react';
...
export default function SettingsDiary() {
  ...
  // ── Initial load (entries with v1→v2 migration, custom games, theme) ──
  useEffect(() => {
    async function loadAll() {
      try {
        ...
        try {
          const gamesData = await window.storage.get('customGames');
          if (gamesData) setCustomGames(JSON.parse(gamesData.value));
        } catch (e) {}
          ...
          for (const key of result.keys) {
            try {
              ...
              if (raw) {
                ...
                // Migration: v1 stored a single object per day; v2 stores an array
                const list = Array.isArray(parsed) ? parsed.map(ensureId) : [ensureId(parsed)];
                if (list.length) loaded[dateKey] = list;
              }
            } catch (e) {}
          }
          setEntries(loaded);
  ...
  const handleSave = async () => {
    ...
    try {
      if (hasData) {
        ...
        await window.storage.set(`entry:${key}`, JSON.stringify(newList));
        setEntries({ ...entries, [key]: newList });
        if (formData.clipFile?.blobUrl) URL.revokeObjectURL(formData.clipFile.blobUrl);
      }
      setSaved(true);
      setTimeout(() => {
  ...
  const addCustomGame = async () => {
    ...
    setGameDropdownOpen(false);
    try {
      await window.storage.set('customGames', JSON.stringify(updated));
    } catch (e) {
  ...
  const applyImport = async () => {
    ...
    try {
      const existing = await window.storage.list('entry:');
      if (existing && existing.keys) {
        for (const key of existing.keys) {
          try { await window.storage.delete(key); } catch (err) {}
        }
      }
      for (const [k, list] of Object.entries(normalized)) {
        await window.storage.set(`entry:${k}`, JSON.stringify(list));
      }
      if (importedGames) {
        await window.storage.set('customGames', JSON.stringify(importedGames));
        setCustomGames(importedGames);
      }
    } catch (err) {
    ...
    <div data-theme={theme} className="sd-root min-h-screen tk-ink" style={{
      ...
      <div className="max-w-5xl mx-auto px-5 sm:px-8 py-8 sm:py-12">
        ...
        <div className="mt-5 text-[9px] tk-faint uppercase flex justify-between flex-wrap gap-2" style={{ letterSpacing: '.18em' }}>
          <span>日付を選択して記録 — データはこのブラウザに保存</span>
